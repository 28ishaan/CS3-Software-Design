...
//     assert(scene_bodies(scene) == 3); // removal is deferred until the next
//     tick scene_tick(scene, 0); assert(scene_bodies(scene) == 2);
//     assert(scene_get_body(scene, 0) == body1);
...
// // A force creator that moves a body in uniform circular motion about the
// origin void centripetal_force(void *aux) {
//     body_t *body = aux;
//     vector_t v = body_get_velocity(body);
//     vector_t r = body_get_centroid(body);
//     assert(isclose(vec_dot(v, r), 0));
//     vector_t force =
//         vec_multiply(-body_get_mass(body) * vec_dot(v, v) / vec_dot(r, r),
//         r);
...
// scene void constant_gravity(void *aux) {
...
//         vector_t force = {0, -gravity_aux->coefficient *
//         body_get_mass(body)}; body_add_force(body, force);
//     }
// }

// // A force creator that applies drag proportional to v ** 2 to all bodies in
// a scene void air_drag(void *aux) {
...
//     scene_t *scene = scene_init();
//     body_t *light = body_init(make_shape(), LIGHT_MASS, (rgb_color_t) {0, 0,
//     0}); scene_add_body(scene, light); body_t *heavy =
//     body_init(make_shape(), HEAVY_MASS, (rgb_color_t) {0, 0, 0});
...
//     // Every time count_calls() is called, the body count should decrease by
//     1 assert(scene_bodies(count_aux->scene) == 3 - count_aux->count);
...
//     scene_t *scene = scene_init();
//     for (int i = 0; i < 3; i++) {
//         scene_add_body(scene, body_init(make_shape(), 1, (rgb_color_t) {0, 0,
//         0}));
//     }
//     scene_add_bodies_force_creator(scene, remove_body, scene, list_init(0,
//     NULL), NULL);
